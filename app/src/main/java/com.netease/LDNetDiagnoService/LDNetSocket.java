...
//  }
   ...
   */
  public boolean exec(StringBuilder result, int port) {
    if (isCConn && loaded) {
    	try{
    		startJNITelnet("80"); //默认80端口
    		return true;
    	}catch(UnsatisfiedLinkError e){
    		e.printStackTrace();
    		Log.i("LDNetSocket", "call jni failed, call execUseJava");
    		return execUseJava(result,port);
    	}	
    } else {
      return execUseJava(result, port);
    }
   ...
   * 使用java执行connected
   */
  private boolean execUseJava(StringBuilder result, int port) {
    if (_remoteInet != null && _remoteIpList != null) {
      ...
      for (int i = 0; i < len; i++) {
        ...
        isConnnected[i] = execIP(_remoteInet[i], _remoteIpList.get(i),result,port);
      }
   ...
   */
  private boolean execIP(InetAddress inetAddress, String ip, StringBuilder result, int port) {
    ...
    if (inetAddress != null && ip != null) {
      ...
      for (int i = 0; i < CONN_TIMES; i++) {
        ...
        if (RttTimes[i] == -1) {// 一旦发生timeOut,则尝试加长连接时间
          ...
          result.append((i + 1) + "'s time=" + "IOException");
          if (i > 0 && RttTimes[i - 1] == -2) {// 连续两次出现IO异常,停止后续测试
            flag = -2;
            break;
          }
        } else {
          result.append((i + 1) + "'s time=" + RttTimes[i]
              + "ms,  ");
        }
      }
      long time = 0;
   ...
   * 调用jni中native方法
   */
  public native void startJNITelnet(String port);
